...
[kc_mid, kc_up_1, kc_down_1] = ta.kc(hlcc4, 50, 2.75)
...
kc_midl = plot(kc_mid, title = 'kc_mid', color = color.blue, force_overlay = true, linewidth = 2)

[kc_mid2, kc_up_2, kc_down_2] = ta.kc(hlcc4, 50, 3.75)

kc_down_2l = plot(kc_down_2, title = 'kc_down_2', color = color.rgb(105, 214, 72, 35), force_overlay = true, linewidth = 2)
...
USDT = request.security('USDT.D', timeframe.period, hlcc4)
z_ursi = hline(50, "z_ursi", color = color.gray, linewidth = 1)
s_ursi = hline(30, "s_ursi", color = color.red, linewidth = 1)
b_ursi = hline(70, "b_ursi", color = color.lime, linewidth = 1)
plot(ta.rsi(USDT, 11), title = 'ursi_1', color = color.aqua, linewidth = 2)
fill(z_ursi, s_ursi, color = color.new(color.lime, 90))
fill(z_ursi, b_ursi, color = color.new(color.red, 90))

DXY = request.security('DXY', timeframe.period, hlcc4)
plot(ta.rsi(DXY, 11), title = 'drsi_1', color = color.purple, linewidth = 1)

NQ = request.security('NQ1!', timeframe.period, hlcc4)
plot(100 - ta.rsi(NQ, 11), title = 'nrsi_1', color = color.maroon, linewidth = 1)
```
## 做多规则
### 抄底规则
